...
to setup
  ...
  ask n-of (97.9 * density / 100) turtles
  [set color red]
  ask n-of (1.1 * density / 100) turtles
  [set color green]
  ask n-of (0.1 * density / 100) turtles
  [set color blue]
  ask n-of (0.9 * density / 100) turtles
  [set color violet]
  ;;ask n-of (6.7 * density / 100) turtles
  ;;[set color yellow]
  update-turtles
  update-globals
  reset-ticks
...
to update-turtles
  ask turtles [
    ...
    set other-nearby count (turtles-on neighbors4) with [ color != [ color ] of myself ]
